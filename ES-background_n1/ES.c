...
#include <SDL/SDL.h>
#include "ES.h"
//backgrund(apsect image seulement)
...
void afficher_ennemi(background *screen,Ennemi imag){
     SDL_BlitSurface(imag.img,&imag.sprite,screen->bg[screen->frame],&imag.pos);
...
/*void animer_ennemi(Ennemi *e)
{
        if(e->dir==0){
		e->sprite.y=151;
		if(e->sprite.x==308-(e->sprite.w)){
			e->sprite.x=0;}
                else{
			e->sprite.x=e->sprite.x+61,6;}}
	else{
		e->sprite.y=0;
		if(e->sprite.x==308-(e->sprite.w)){
			e->sprite.x=0;}
